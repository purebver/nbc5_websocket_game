...
function showStageInfoText(stageId) {
  const fontSize = 70 * scaleRatio;
  ctx.font = `${fontSize}px Verdana`;
  ctx.fillStyle = 'grey';
  const x = canvas.width / 3;
  const y = canvas.height / 2;
  ctx.fillText(`Stage: ${stageId}`, x, y);
}
...
function updateGameSpeed(deltaTime, maxSpeed) {
  if (maxSpeed === score.leng) {
    gameSpeed += deltaTime * GAME_SPEED_INCREMENT;
  } else {
    if (gameSpeed < GAME_SPEED_START + maxSpeed) {
      gameSpeed += deltaTime * GAME_SPEED_INCREMENT * 3;
    } else {
      gameSpeed = GAME_SPEED_START + maxSpeed;
    }
  }
}
